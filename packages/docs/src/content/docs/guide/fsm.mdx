...
| `canHandle(inputName)`       | `true` if the current state has a handler (or `"*"`) for this input, or if the current state's `_child` chain can handle it.                       |
...
Useful for conditionally triggering inputs without swallowing `nohandler` events. When the current state mounts a child FSM, `canHandle()` answers for the whole `_child` chain — the same reach `handle()`'s delegation has:
...
<Aside type="note">
    A shorthand typo like `timeout: "yellw"` is always caught, but *where* TypeScript anchors the
    error varies with the shape of the neighboring states: it can land on the containing state
    object's line (e.g. `green: {`) or directly on the `timeout` property itself. Either way, the
    error chain names the exact property and ends with a "Did you mean" suggestion — if the first
    line looks broad, read past it.
</Aside>
