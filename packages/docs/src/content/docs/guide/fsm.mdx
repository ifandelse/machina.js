...
If you need one FSM definition to manage many independent client objects, see [createBehavioralFsm](/machina.js/guide/behavioral-fsm/).
...
| `defer`     | Queues the current input for replay after the next transition. See [Deferred Input](/machina.js/guide/defer/). |
...
See [Events](/machina.js/guide/events/) for the full event reference.
