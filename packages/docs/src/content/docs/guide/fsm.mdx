...
<Aside type="note">
    A shorthand typo like `timeout: "yellw"` is reported by TypeScript on the *state object's* line
    (e.g. `green: {`), not on the `timeout` property itself. This is a consequence of how state
    validation works: every state shares one combined shape, so a mismatch anywhere inside it
    surfaces as one incompatibility on the containing state. The error chain underneath still names
    the exact property and a "Did you mean" suggestion — read past the first line.
</Aside>
