...
- **Validation with no blind spots**: empty states (`{}`) and states containing only lifecycle hooks used to silently disable type checking for the whole config. Every config shape is now fully validated — latent typos surface as compile errors on upgrade. See the [migration notes](/migration/v6-to-v7/).
- **`defer({ until })` is typed**: the `until` target only accepts declared state names, with a "Did you mean" suggestion on typos.
- **Parents accept child inputs**: `handle()` on a parent type-checks with a child's — or grandchild's — inputs, no casts, and delegation now reaches any depth at runtime.
- **`bubbles` declarations**: an FSM names the inputs it fires at itself but never handles; whatever mounts it via `_child` must provably cover them or the config fails to compile.
- **Readable compiler errors**: rejected `handle()`/`transition()` calls show the flat list of valid names instead of an unresolved type alias.
