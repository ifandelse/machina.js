...
<Aside type="note">
    Like `createFsm`, a shorthand typo (`connect: "conecting"`) is reported on the *state object's*
    line rather than the property itself — every state shares one combined validated shape, so the
    error chain names the specific property a level down. See the [`createFsm` TypeScript
