...
- [Shopping Cart](/examples/shopping-cart/) — targeted defer across multiple transitions (`recordPurchaseAnalytics` queued from `browsing` through `validating` and `applyingDiscount` all the way to `checkout`), plus untargeted defer in the `error` state's `"*"` catch-all.
- [Traffic Intersection](/examples/traffic-intersection/) — pedestrian button press deferred from `green` to `interruptibleGreen`; if the button is pressed too early in the green phase, it queues and automatically shortens the interruptible portion when that window opens.
