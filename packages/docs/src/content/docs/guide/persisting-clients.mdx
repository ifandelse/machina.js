...
<Aside type="note">
    See it in a working app: the [Job Queue example](/examples/job-queue/) persists every job to
    `localStorage` as a `dehydrate()` snapshot rather than a bare state string, so a job's pending
    deferred input survives a page reload and replays correctly after `rehydrate()`.
</Aside>
