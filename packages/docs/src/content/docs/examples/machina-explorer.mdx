...
machina-explorer is a browser-based tool for validating FSM configs and visualizing their state graphs. Paste a config object, click **Analyze**, and the app runs machina-inspect checks and renders a mermaid state diagram — no build step, no backend.
