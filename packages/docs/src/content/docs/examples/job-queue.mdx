...
- **Deferred inputs survive the round trip** — a still-`queued` job can receive a pre-emptive `pause`, deferred until it reaches `processing`. A bare state string can't carry that; the snapshot form does. See [Persisting clients](/guide/persisting-clients/) for the general pattern.
...
| State        | What happens                                                                                                                                                                                                  |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `queued`     | Registered but not running. `start` transitions to `processing`. `pause` defers itself until `processing` is reached — a pre-emptive pause, and the deferred input that demonstrates the snapshot round trip. |
